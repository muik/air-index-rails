class MeasuresController < AdminController
  ...
  # GET /measures.json
  def index
    @measures = Measure.all.limit(50).order_by(:time.desc)
    @station = Station.find params['station_id'] if params['station_id']
    @measures = @measures.where(station: @station) if @station
  ...
  private
    ...
    def measure_params
      params.require(:measure).permit(:station, :time, :grade, :index)
    end
end
